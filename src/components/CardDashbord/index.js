...
const CardHabits = ({ habits, setChange }) => {
    ...
    const { title, category, difficulty, frequency, achieved, how_much_achieved, user, id } = habits;
    ...
    const changeAchieved = () => {
        if (achieved === false) {
            api.patch(`/habits/${id}/`, habitConclude, { headers: { Authorization: `Bearer ${token}` } })
                .then((_) => toast.success("Hábito concluído!"))
                .catch((_) => toast.error("Erro ao alterar!"))
            setChange(true)
        }
        if (achieved === true) {
            api.patch(`/habits/${id}/`, habitNotConclude, { headers: { Authorization: `Bearer ${token}` } })
    ...
    const [checked, setChecked] = useState(achieved);
                ...
                <ButtonContainer>
                    ...
                    <input type="checkbox" onChange={(e) => { setChecked(!achieved) }} checked={checked} onClick={() => { changeAchieved() }} ></input>
                </ButtonContainer>
            </DataButtonDiv>
        </Container>
    )
